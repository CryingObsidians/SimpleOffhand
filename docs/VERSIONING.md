...
`runs { data { ... } }` 里写错会在配置阶段直接失败（`Trying to prepare unknown run: xxx`），
表现为 IDE 同步失败：
...
`clientData` / `serverData` 是 1.21.8 引入的，且**同线不存在 `data`**；反过来旧线只有 `data`。
...
**文件名写错会让模组静默不加载** —— FML 只会去找它认识的文件名，其余一律忽略，
既不报错也不出现在模组列表里。
...
- **各分支的注入是否真的生效只验证过一部分**：`1.21.1` / `1.21.8` 进世界确认过；
  `26.3` 只验证到"模组被加载"，还没进世界看手臂。26.3 的目标类刚被重构，尤其需要实测。
- **26.1.0 / 26.1.1 的方法名未核对**：26.1.2 是 `renderArmWithItem`，同线更早版本没验证过。
- **渲染线程每帧读配置**：`SimpleOffhandConfig` 的两个方法每帧各调一次 `ConfigValue#get()`。
  暂不处理；真要做就监听 `ModConfigEvent.Loading` 把值读进字段，顺带消掉"配置未加载即读取"会抛
  `IllegalStateException` 的隐患。
- **注入是否真的生效只能进游戏看**：方法名或参数类型写错**不会编译报错**，运行期才抛
  `Mixin apply failed`。进游戏、空着副手看第一人称，日志里出现
